...
if ! systemctl is-active --quiet business-monitor.service; then
    echo "FAIL: Business monitor service should be running after restart" >> /tmp/progress.log
    exit 1
fi

# Verify there are no deleted files with lsof
if lsof +L1 /var/log 2>/dev/null | grep -q business-monitor; then
    echo "FAIL: Deleted file should be cleaned up (no files with deleted marker)" >> /tmp/progress.log
    exit 1
fi
