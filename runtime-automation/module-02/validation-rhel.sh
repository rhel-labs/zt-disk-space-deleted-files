...
if [ -f /var/log/super-business/business-monitor.log ]; then
    ...
    echo "HINT: Use 'rm /var/log/super-business/business-monitor.log' to delete it" >> /tmp/progress.log
    exit 1
fi
...
if ! ls -l /proc/$BUSINESS_PID/fd/ 2>/dev/null | grep -q deleted; then
    echo "FAIL: Process should still have the deleted file open" >> /tmp/progress.log
    exit 1
fi
