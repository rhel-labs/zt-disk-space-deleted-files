= Disk Space Troubleshooting: Deleted Files
...
* Diagnose disk space issues using `df` and `du`
* Understand how Linux handles file deletion and file descriptors
* Use `lsof` to find deleted files that are still open
...
* Use the utilities in the `utilities/` directory
* Run `./utilities/lab-build` to build the documentation
* Run `./utilities/lab-serve` to preview locally
